...
class Spine(Node):
    ...
    def _add_neighbours(self, pod_number, level, pod_total_levels, connected_leafs, connected_tofs, connected_spines):
        """
        ...
        if real_level == 1:
            ...
            for leaf_num in range(1, connected_leafs + 1):
                leaf_name = 'leaf_%d_0_%d' % (pod_number, leaf_num)
            ...
            if pod_total_levels == 1:
                ...
                for spine_num in range(1, connected_spines[1] + 1):
                    spine_name = 'spine_%d_%d_%d' % (pod_number, real_level + 1, spine_num)
            ...
            for spine_num in range(1, connected_spines[level - 1] + 1):
                southern_spine_name = 'spine_%d_%d_%d' % (pod_number, real_level - 1, spine_num)
            ...
            if real_level == pod_total_levels:
                ...
                for spine_num in range(1, connected_spines[level + 1]):
                    northern_spine_name = 'spine_%d_%d_%d' % (pod_number, real_level + 1, spine_num)
